var basicBlog = {
    ...
    updateIndex: function() {
        var html = "";
        $.getJSON("blogs/js/database.json", function(data) {
            var i = 0;
            while (i < data.length) {
                html += '<div class="col-md-4"><div class="post-preview"><h3 class="post-title"><a href="blogs/post.html?post=' + data[i].slug + '">' + data[i].title + '</a></h3><h4 class="post-subtitle">' + data[i].summary + '</h4><p class="post-meta">Posted on ' + basicBlog.getDate(data[i].posted_on) + "</p></div></div>";
                i++;
            }
